  ...
  try {
    ...
    if (customerColumns.length > 0) {
      ...
      if (fkRows.length > 0) {
        ...
        await connection.execute(`ALTER TABLE sales_orders DROP FOREIGN KEY \`${fkName}\``);
        
      }

      // Drop index if exists
      try {
        await connection.execute(`ALTER TABLE sales_orders DROP INDEX idx_sales_orders_customer_id`);
        
      } catch (e) {
        
      }

      await connection.execute(`ALTER TABLE sales_orders DROP COLUMN customer_id`);
