...
//   onSubmitted = null,
// }) {
//   // Minimal local state and helpers (rest of the file expects these to exist).
...
//               unit: r.unit || product?.unit, // Preserve unit from item or product
//             };
//           });
...
//                 d1_percent: Number(r.discount_rate) || 0,
//                 gst_percent: Number(r.gst_percent ?? product?.gst_percent ?? 0),
//                 unit: r.unit || product?.unit, // Add unit here
...
//     const base = Number(row.cost_rate) || 0; // this is product.value (base cost)
//     const buyerType = header.buyer_type || "";
...
//         const explicitTotal = Number(row.total_50 || 0);
//         const explicitDisc50 = Number(row.discount_50 || 0);
...
//         selling = base + (Number(row.discount_30) || 0);
//       } else if (q >= 10) {
//         // Wholesale qty >= 10 → base + discount_25
//         selling = base + (Number(row.discount_25) || 0);
//       }

//       return Number.isFinite(selling) && selling > 0 ? selling : base;
//     }
//     return row.rate || 0;
//   };

//   // Recompute rates when buyer type changes for non-manual rows
//   useEffect(() => {
//     setRows((prev) =>
//       prev.map((r) => {
//         if (useCostMargin && !r.manualRate) {
//           const newRate = recomputeSellingRate(r);
...
//             icon: "info",
//             title: "Stock limit",
//             text: "Qty limited to available stock",
//           });
//         }
...
//               const perUnitDisc =
//                 (Number(r.rate || 0) * (Number(r.d1_percent) || 0)) / 100;
...
const getRateForUnit = (ratePerKg, unit) => {
  if (!unit || !UNIT_CONVERSIONS[unit.toLowerCase()]) return ratePerKg;

  // Convert rate per kg to rate per unit
  const ratePerGram = ratePerKg / 1000; // Rs per gram
  return ratePerGram * UNIT_CONVERSIONS[unit.toLowerCase()];
};

// Helper function to convert rate from per unit to per kg
const getRatePerKgFromUnit = (ratePerUnit, unit) => {
  if (!unit || !UNIT_CONVERSIONS[unit.toLowerCase()]) return ratePerUnit;

  // Convert rate per unit to rate per kg
  const ratePerGram = ratePerUnit / UNIT_CONVERSIONS[unit.toLowerCase()];
...
export default function SalesForm({
  ...
  const emptyRow = {
    ...
    rate: 0, // Rate per selected unit (display)
  ...
  const recomputeSellingRate = (row) => {
    ...
    const qtyInKg =
      row.qty_in_kg || convertToGrams(Number(row.qty || 0), row.unit) / 1000;
    ...
    if (useCostMargin && !row.manualRate) {
      ...
      let sellingPerKg = base;

      if (!isWholesale || qtyInKg < 5) {
        // Default 50% margin case – use explicit total/discount_50 if available
        const explicitTotal = Number(row.total_50 || 0);
        const explicitDisc50 = Number(row.discount_50 || 0);

        if (explicitTotal > 0) {
          sellingPerKg = explicitTotal;
        } else if (explicitDisc50 > 0) {
          sellingPerKg = base + explicitDisc50;
        } else {
          sellingPerKg = base * 1.5;
        }
      } else if (qtyInKg >= 5 && qtyInKg < 10) {
        // Wholesale 5–9 qty → base + discount_30
        sellingPerKg = base + (Number(row.discount_30) || 0);
      } else if (qtyInKg >= 10) {
        // Wholesale qty >= 10 → base + discount_25
        sellingPerKg = base + (Number(row.discount_25) || 0);
      }

      return Number.isFinite(sellingPerKg) && sellingPerKg > 0
        ? sellingPerKg
        : base;
    }
    return row.rate_per_kg || 0;
  };

  const calc = (r) => {
    const qtyInKg =
      Number(r.qty_in_kg) || convertToGrams(Number(r.qty || 0), r.unit) / 1000;
    const ratePerKg =
      Number(r.rate_per_kg) ||
      getRatePerKgFromUnit(Number(r.rate || 0), r.unit);
        ...
        if (isEditMode && sale) {
          ...
          const mapped = (sale.items || []).map((r) => {
            ...
            return {
              ...
              qty: Number(r.qty) || 1,
              qty_in_kg: convertToGrams(Number(r.qty) || 0, unit) / 1000,
              cost_rate: Number(product?.cost_rate ?? 0),
              rate_per_kg: Number(r.rate ?? 0),
              rate: getRateForUnit(Number(r.rate ?? 0), unit),
              manualRate: true,
              d1_percent: Number(r.discount_rate) || 0,
              gst_percent: Number(r.gst_percent ?? product?.gst_percent ?? 0),
              unit: unit,
  ...
  useEffect(() => {
    setRows((prev) =>
      prev.map((r) => {
        if (useCostMargin && !r.manualRate) {
          const newRatePerKg = recomputeSellingRate(r);
          const newRate = getRateForUnit(newRatePerKg, r.unit);
          return { ...r, rate_per_kg: newRatePerKg, rate: newRate };
        }
        return r;
      })
    );
    ...
    setRows((prev) => {
      ...
      if (field === "product_id") {
        ...
        if (product) {
          ...
          next[i].unit = product.unit || "kg"; // Set initial unit from product
          next[i].discount_25 = Number(product.discount_25 || 0);
          next[i].discount_30 = Number(product.discount_30 || 0);
          next[i].discount_50 = Number(product.discount_50 || 0);
          next[i].total_50 = Number(product.total_50 || 0);
          ...
          // Set initial rate
          if (
            (!next[i].rate_per_kg || next[i].rate_per_kg === 0) &&
            product.cost_rate > 0
          ) {
            ...
            next[i].rate_per_kg = initialRatePerKg;
            next[i].rate = getRateForUnit(initialRatePerKg, next[i].unit);
            next[i].manualRate = false;
          } else if (product.cost_rate != null) {
            // If product has a sale rate from database (per kg)
            next[i].rate_per_kg = product.cost_rate; // This is per kg
            next[i].rate = getRateForUnit(product.cost_rate, next[i].unit);
            next[i].manualRate = true;
          }

          // Calculate qty_in_kg for the initial qty
          next[i].qty_in_kg = convertToGrams(next[i].qty, next[i].unit) / 1000;
        }
      }
      ...
      if (field === "qty") {
        ...
        next[i].qty = q;
        next[i].qty_in_kg = convertToGrams(q, next[i].unit) / 1000; // Convert to kg for calculations

        // Recalculate rate if using cost margin and not manual
        if (useCostMargin && !next[i].manualRate) {
          const tmp = { ...next[i], qty: q };
          const newRatePerKg = recomputeSellingRate(tmp);
          next[i].rate_per_kg = newRatePerKg;
          next[i].rate = getRateForUnit(newRatePerKg, next[i].unit);
        }
      }

      if (field === "rate") {
        next[i].manualRate = true;
        next[i].rate_per_kg = getRatePerKgFromUnit(
          Number(value) || 0,
          next[i].unit
        );
      }
      ...
      if (field === "cost_rate" && useCostMargin && !next[i].manualRate) {
        const newRatePerKg = recomputeSellingRate(next[i]);
        next[i].rate_per_kg = newRatePerKg;
        next[i].rate = getRateForUnit(newRatePerKg, next[i].unit);
      }

      // When unit changes, update rate display
      if (field === "unit") {
        const oldUnit = next[i].unit;
        next[i].unit = value;

        // Convert rate display to new unit
        if (next[i].rate_per_kg > 0) {
          next[i].rate = getRateForUnit(next[i].rate_per_kg, value);
        }
        ...
        if (
          ...
          const qtyInGrams = convertToGrams(next[i].qty, oldUnit);
          next[i].qty = convertFromGrams(qtyInGrams, value);
          next[i].qty_in_kg = qtyInGrams / 1000;
        }
        ...
        if (next[i].qty > availInSelectedUnit) {
          ...
          next[i].qty_in_kg = convertToGrams(availInSelectedUnit, value) / 1000;
          Swal.fire({
            icon: "info",
            title: "Stock limit",
            text: `Qty limited to available stock (${fx(
    ...
    try {
      ...
      const payload = {
        ...
        items: rows.map((r) => ({
          ...
          unit: r.unit || "kg",
          rate: Number(
            r.rate_per_kg || getRatePerKgFromUnit(r.rate || 0, r.unit)
          ), // Send rate per kg
          display_rate: Number(r.rate || 0), // Send display rate
        })),
      };
                  ...
                  <td className="px-2 py-1 border text-right">
                    ...
                    <div className="text-[10px] text-gray-500 text-right">
                      {useCostMargin && !r.manualRate
                        ? `Margin: ${getMarginPercentByQty(
                            r.qty_in_kg ||
                              convertToGrams(Number(r.qty || 0), r.unit) / 1000,
                            header.buyer_type
                          )}% ${
                            header.buyer_type === "Retailer"
                              ? "(fixed)"
                        ...
                        : "Manual rate"}
                      <br />
                      {r.unit ? `Per ${r.unit}` : "Per kg"} |{" "}
                      {fx(
                        r.rate_per_kg ||
                          getRatePerKgFromUnit(r.rate || 0, r.unit),
                        2
                      )}
                      /kg
                    </div>
...
//     hsn_code: "",
//     available_grams: 0, // Available quantity in grams
//     available_display: 0, // Available quantity in selected unit for display
//     qty: 1,
//     qty_in_kg: 0, // Quantity in kg for calculations
//     cost_rate: 0, // Cost per kg
//     rate: 0, // Rate per selected unit (display)
...
//               hsn_code: product?.hsn_code || r.hsn_code || "",
//               available_grams: availableGrams,
//               available_display: convertFromGrams(availableGrams, unit),
//               qty: Number(r.qty) || 1,
//               qty_in_kg: convertToGrams(Number(r.qty) || 0, unit) / 1000,
//               cost_rate: Number(product?.cost_rate ?? 0),
//               rate_per_kg: Number(r.rate ?? 0),
...
//           // Convert available grams to selected unit for display
//           next[i].available_display = convertFromGrams(
//             next[i].available_grams,
//             next[i].unit
//           );
...
//       if (field === "qty") {
//         const availDisplay = Number(next[i].available_display || 0);
//         let q = Number(value || 0);
...
//             icon: "info",
//             title: "Stock limit",
//             text: `Qty limited to available stock (${fx(availDisplay, 3)} ${
//               next[i].unit
//             })`,
//           });
//         }
...
//       // When unit changes, update rate display and available display
//       if (field === "unit") {
//         const oldUnit = next[i].unit;
//         next[i].unit = value;
...
//         // Convert available quantity display
//         if (next[i].available_grams > 0) {
//           next[i].available_display = convertFromGrams(
//             next[i].available_grams,
//             value
//           );
...
//         // Validate quantity doesn't exceed available in new unit
//         const availDisplay = convertFromGrams(next[i].available_grams, value);
//         if (next[i].qty > availDisplay) {
//           next[i].qty = availDisplay;
//           next[i].qty_in_kg = convertToGrams(availDisplay, value) / 1000;
//           Swal.fire({
//             icon: "info",
//             title: "Stock limit",
//             text: `Qty limited to available stock (${fx(
...
//                       value={fx(
//                         r.available_display ||
//                           convertFromGrams(r.available_grams || 0, r.unit),
//                         3
//                       )}
//                     />
//                     <div className="text-[8px] text-gray-500">
//                       {r.unit ? `in ${r.unit}` : "in kg"}
//                     </div>
//                   </td>
//                   <td className="px-2 py-1 border text-center">
//                     <input
