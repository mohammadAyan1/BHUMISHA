...
//     // IMPORTANT: Check if total_50 exists - that's the actual selling price for 50% margin
//     const total50 = Number(row.total_50 || 0);
...
//       // If total_50 is available, use it as the base selling price (already includes 50% margin)
//       if (total50 > 0) {
//         sellingPerKg = total50; // This is already the selling price per kg with 50% margin
//       } else if (!isWholesale || qtyInKg < 5) {
//         // Default 50% margin case – use explicit total/discount_50 if available
//         const explicitTotal = Number(row.total_50 || 0);
//         const explicitDisc50 = Number(row.discount_50 || 0);
...
//           sellingPerKg = base + explicitDisc50;
//         } else {
//           sellingPerKg = base * 1.5; // 50% markup
//         }
//       } else if (qtyInKg >= 5 && qtyInKg < 10) {
//         // Wholesale 5–9 qty → base + discount_30
//         sellingPerKg = base + (Number(row.discount_30) || 0);
    ...
    setRows((prev) => {
      ...
      if (field === "product_id") {
        ...
        if (product) {
          ...
          // Set initial rate (always per kg)
          if ((!next[i].rate || next[i].rate === 0) && product.cost_rate > 0) {
            ...
            // If product has a sale rate from database (per kg)
            next[i].rate = product.cost_rate; // This is per kg
            next[i].manualRate = true;
          }
      ...
      if (field === "qty") {
        ...
        next[i].qty = q;
        next[i].qty_in_kg = convertUnitToKG(q, next[i].unit); // Convert to kg for calculations

        // Recalculate rate if using cost margin and not manual
        if (useCostMargin && !next[i].manualRate) {
          const tmp = { ...next[i], qty: q };
          const newRate = recomputeSellingRate(tmp);
          next[i].rate = newRate;
        }
      }
      ...
      if (field === "rate") {
        next[i].manualRate = true;
        // Rate is always per kg, so no conversion needed
      }

      if (field === "cost_rate" && useCostMargin && !next[i].manualRate) {
        const newRate = recomputeSellingRate(next[i]);
        next[i].rate = newRate;
      }
