...
export default function PurchaseList({ reload }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        const partyName =
          params.row.party_name || params.row.vendor_name || "-";
    ...
    },
    {
      ...
      width: 140,
      renderCell: (params) => {
        // Calculate total quantity from items with units
        const quantities =
          params.row.items?.reduce((acc, item) => {
            const quantity = Number(item.size || item.quantity || 0);
            const unit = item.unit || "";
            const unitDisplay = getUnitDisplay(unit);
        ...
        return (
          <div className="space-y-1">
            {Object.entries(quantities).map(([unit, qty], index) => (
              <div key={index} className="flex items-center gap-2">
                <ScaleIcon className="text-gray-500" fontSize="small" />
                <span className="font-medium">{fx(qty)}</span>
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        items.forEach((item) => {
          ...
          const quantityInKg = convertToKg(quantity, unit);
          const itemValue = calculateItemValue(quantity, unit, rate);

          totalValue += itemValue;
          totalKg += quantityInKg;
        });

        const avgRate = totalKg > 0 ? totalValue / totalKg : 0;

        return (
          <div className="text-right">
            <div className="font-semibold">₹{fx(avgRate)}</div>
    ...
    },
    {
      ...
      width: 120,
      renderCell: (params) => {
        if (!params.row.transport_name)
          return <span className="text-gray-400">-</span>;
        return (
          <div className="flex items-center gap-1 text-sm">
            <LocalShippingIcon fontSize="small" className="text-gray-500" />
            <span className="truncate" title={params.row.transport_name}>
              {params.row.transport_name}
            </span>
          </div>
        );
      },
