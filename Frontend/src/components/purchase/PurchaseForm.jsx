...
//       item_name: "",
//       hsn_code: "",
//       originalAvailable: 0, // Store original product stock
//       purchaseQuantity: 0, // Store quantity being purchased
//       available: 0,
//       size: 0,
//       rate: 0,
//       d1_percent: 0,
...
//         return Number(r.available || 0) === avail
//           ? r
//           : { ...r, available: avail };
//       })
//     );
//     // NOTE: rows ko dependency me mat rakho, warna loop hoga
//   }, [products]);

//   useEffect(() => {
//     let ran = false;
//     const prefillFromPO = async () => {
//       if (ran || !isCreateModeFromPO) return;
//       ran = true;
//       // ... existing prefill code
//     };
//     prefillFromPO();
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [isCreateModeFromPO, poIdFromQuery]);
...
//         console.log(res, "this is convert po  to p");
...
//         // Bill image preview from server URL (e.g., /public/uploads/...)
...
//     load();
//     // keep products in deps if you want product-derived fields to rebind when product list comes later
//     // eslint-disable-next-line react-hooks/exhaustive-deps
...
//   const onRowFile = (i, file) => {
//     if (!file) {
//       setRows((prev) => {
//         const n = [...prev];
//         n[i] = { ...n[i], image_file: null, image_preview: "" };
//         return n;
//       });
//       return;
//     }
...
//     setRows((prev) => {
//       const n = [...prev];
//       n[i] = { ...n[i], image_file: file, image_preview: url };
//       return n;
//     });
//   };
...
//         next[i].originalAvailable = Number(p?.available || 0); // Store original stock
...
//             (x) => Number(x.id) === Number(it.product_id)
//           );
//           console.log(p, "this is the Product data ");
//           const ChangesRate = Number(it?.rate);

//           const lt = Number(p?.raw?.local_transport);
//           const pc = Number(p?.raw?.packaging_cost);
//           const tc = Number(p?.raw?.transport_charge);
...
//           console.log(it?.unit);
//           console.log(p?.unit);
//           // FIXED: Handle size calculation properly
//           let changeProductSize = Number(p?.size) || 0;
//           const purchaseSize = Number(it?.size) || 0;

//           if (it?.unit == "ton") {
//             // Convert tons to grams: 1 ton = 1000 kg = 1,000,000 grams
//             const purchaseSizeInGrams = purchaseSize * 1000 * 1000;
//             changeProductSize = changeProductSize + purchaseSizeInGrams;
//           } else if (it?.unit == "quantal") {
//             // Convert quantal to grams: 1 quantal = 100 kg = 100,000 grams
//             const purchaseSizeInGrams = purchaseSize * 100 * 1000;
//             changeProductSize = changeProductSize + purchaseSizeInGrams;
//           } else if (it?.unit == "kg") {
//             // Convert kg to grams: 1 kg = 1000 grams
//             const purchaseSizeInGrams = purchaseSize * 1000;
//             changeProductSize = changeProductSize + purchaseSizeInGrams;
//           } else if (it?.unit == "gram") {
//             // Already in grams
//             changeProductSize = changeProductSize + purchaseSize;
//           } else {
...
//           console.log("====================================");
//           console.log("Updated Product Size:", changeProductSize);
//           console.log("====================================");
...
//             discount_30: Number(margin_30),
//             discount_25: Number(margin_25),
//             discount_50: Number(margin_50),
//             total: Number(totalSellingValue),
//             gst: it?.gst_percent,
//             size: changeProductSize,
//           });
//         });
//         await Promise.allSettled(updates);
//         await dispatch(fetchProducts());
...
//           <tbody>
//             {rows.map((r, i) => {
//               let base = (r.size || 0) * (r.rate || 0);
//               let perUnitDisc = ((r.rate || 0) * (r.d1_percent || 0)) / 100;
//               let totalDisc = (r.size || 0) * perUnitDisc;
//               let taxable = Math.max(0, base - totalDisc);
//               let gstAmt = (taxable * (r.gst_percent || 0)) / 100;
//               let final = taxable + gstAmt;

//               let sizeAccordinngToUnit;

//               if (r?.unit == "ton") {
//                 sizeAccordinngToUnit = r?.available / (1000 * 1000);
//                 base = (r.size || 0) * (r.rate || 0) * 1000;
//                 perUnitDisc =
//                   (((r.rate || 0) * (r.d1_percent || 0)) / 100) * 1000;
//                 totalDisc = (r.size || 0) * perUnitDisc;
//                 taxable = Math.max(0, base - totalDisc);
//                 gstAmt = (taxable * (r.gst_percent || 0)) / 100;
//                 final = taxable + gstAmt;
//               } else if (r?.unit == "quantal") {
//                 sizeAccordinngToUnit = r?.available / (1000 * 100);
//                 base = (r.size || 0) * (r.rate || 0) * 100;
//                 perUnitDisc =
//                   (((r.rate || 0) * (r.d1_percent || 0)) / 100) * 100;
//                 totalDisc = (r.size || 0) * perUnitDisc;
//                 taxable = Math.max(0, base - totalDisc);
//                 gstAmt = (taxable * (r.gst_percent || 0)) / 100;
//                 final = taxable + gstAmt;
//               } else if (r?.unit == "kg") {
//                 sizeAccordinngToUnit = r?.available / 1000;
//               } else {
//                 sizeAccordinngToUnit = r?.available;
//                 base = ((r.size || 0) * (r.rate || 0)) / 1000;
//                 perUnitDisc =
//                   ((r.rate || 0) * (r.d1_percent || 0)) / 100 / 1000;
//                 totalDisc = (r.size || 0) * perUnitDisc;
...
//       item_name: "",
//       hsn_code: "",
//       originalAvailable: 0, // Original stock from product
//       size: 0, // Purchase quantity
//       rate: 0,
//       d1_percent: 0,
//       gst_percent: 0,
//       image_preview: "",
...
//             item_name: it.item_name || "",
//             hsn_code: it.hsn_code || "",
//             originalAvailable: Number(p?.available || 0), // Store original stock
...
//           next[i].gst_percent = Number(p.gst_percent || 0);
//           next[i].unit = p.unit || "";
//           // Keep the existing purchase quantity (size) if already set
//         }
//       }
...
//           console.log(purchaseSizeInGrams);
//           console.log("====================================");

//           // Calculate new product size (ADD to existing)
...
//           console.log("- Current Size:", currentProductSize, "grams");
//           console.log("- Purchase:", it?.size, it?.unit);
//           console.log("- Purchase in Grams:", purchaseSizeInGrams, "grams");
//           console.log("- New Total Size:", newProductSize, "grams");
...
//             size: newProductSize, // Updated size
//           });
//         });
...
//           <tbody>
//             {rows.map((r, i) => {
//               // Calculate total available: original stock + new purchase
//               const totalAvailable =
//                 (Number(r.originalAvailable) || 0) + (Number(r.size) || 0);

//               let displayAvailable;
//               let base, perUnitDisc, totalDisc, taxable, gstAmt, final;

//               if (r?.unit == "ton") {
//                 displayAvailable = totalAvailable / (1000 * 1000); // Convert grams to tons for display
//                 base = (r.size || 0) * (r.rate || 0) * 1000;
//                 perUnitDisc =
//                   (((r.rate || 0) * (r.d1_percent || 0)) / 100) * 1000;
//                 totalDisc = (r.size || 0) * perUnitDisc;
...
//                 final = taxable + gstAmt;
//               } else if (r?.unit == "quantal") {
//                 displayAvailable = totalAvailable / (1000 * 100); // Convert grams to quantal
//                 base = (r.size || 0) * (r.rate || 0) * 100;
//                 perUnitDisc =
//                   (((r.rate || 0) * (r.d1_percent || 0)) / 100) * 100;
//                 totalDisc = (r.size || 0) * perUnitDisc;
...
//                 final = taxable + gstAmt;
//               } else if (r?.unit == "kg") {
//                 displayAvailable = totalAvailable / 1000; // Convert grams to kg
//                 base = (r.size || 0) * (r.rate || 0);
//                 perUnitDisc = ((r.rate || 0) * (r.d1_percent || 0)) / 100;
//                 totalDisc = (r.size || 0) * perUnitDisc;
//                 taxable = Math.max(0, base - totalDisc);
//                 gstAmt = (taxable * (r.gst_percent || 0)) / 100;
//                 final = taxable + gstAmt;
//               } else {
//                 displayAvailable = totalAvailable; // Already in grams
//                 base = ((r.size || 0) * (r.rate || 0)) / 1000;
//                 perUnitDisc =
//                   ((r.rate || 0) * (r.d1_percent || 0)) / 100 / 1000;
//                 totalDisc = (r.size || 0) * perUnitDisc;
...
//                       readOnly
//                       className="border rounded p-1 w-20 bg-gray-100"
//                       value={fx(displayAvailable)}
//                       title={`Total: ${totalAvailable} grams`}
//                     />
//                   </td>
    ...
    try {
      ...
      if (isEditMode) {
        ...
        const updates = payload.items.map((it) => {
          ...
          console.log("====================================");
          console.log("Product:", p?.product_name);
          console.log("Current Stock:", currentProductSize, "grams");
          console.log("Purchase:", it?.size, it?.unit);
          console.log("Purchase in Grams:", purchaseSizeInGrams, "grams");
          console.log("====================================");
