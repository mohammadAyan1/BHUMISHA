...
const SalesReports = () => {
            ...
            .flatMap((company) => {
              ...
              return entries.map((entry) => {
                ...
                const totalAmt = itemsArr.reduce(
                  (s, p) =>
                    s + Number(p?.total ?? p?.net_total ?? p?.amount ?? 0),
                  0
                );
  ...
  const filtered = useMemo(() => {
    ...
    let arr = salesBills || [];
    if (companyFilter) arr = arr.filter((b) => b.companyCode === companyFilter);
    if (partyType)
      arr = arr.filter(
        (b) =>
    ...
    return arr;
  }, [salesBills, companyFilter, partyType, dateFrom, dateTo, search]);

  const totalAmount = useMemo(
    () => filtered.reduce((s, b) => s + Number(b.total_amount || 0), 0),
    [filtered]
  );
  ...
  const handleExpand = (bill) => setModalData(bill);
  const handleCloseModal = () => setModalData(null);

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Sales Reports</h1>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <input
                type="text"
                placeholder="Search by product, customer, vendor, bill no, HSN, category..."
                value={search}
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-right text-gray-700">
                            {first?.purchase_rate ?? first?.rate ?? 0}
                          </td>
                      ...
                      <tfoot className="bg-gray-100 border-t-2 border-gray-300 font-bold">
                        <tr>
                          ...
                          <td className="px-4 py-3 text-right text-lg text-blue-600">
                            {(Number(modalData.total_amount) || 0).toFixed(2)}
                          </td>
                        </tr>
                      </tfoot>
                    </table>
